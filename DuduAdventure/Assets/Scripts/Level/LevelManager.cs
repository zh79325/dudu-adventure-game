...
namespace DuduAdventure.Level
{
    ...
    public class LevelManager : MonoBehaviour
    {
        ...
        {
            // 不要用 GetInstanceID()：Unity 6.3 起它已废弃（编译报 CS0619），
            // 替代的 GetEntityId() 返回 64 位 EntityId，塞不进 int。
            // 而且实例 ID 每次运行都会变，本来就不适合当存档里的检查点标识——
            // Checkpoint 组件自己带序列化的唯一 ID，用它才对。
            var component = checkpoint.GetComponent<Checkpoint>();
            ActivateCheckpoint(
                checkpoint.transform.position,
                component != null ? component.CheckpointId : -1);
        }
    ...
    public class Checkpoint : MonoBehaviour
    {
        ...
        /// <summary>检查点唯一 ID（同一关卡中应唯一）</summary>
